...
varmeth <- read.table("varmeth_final_colored.txt", sep="\t", head=T)
head(varmeth)
...
p<-ggplot(data, aes(Temperature, Locus_Location)) + geom_tile(aes(fill = Status),
                colour = "gray22") + scale_y_discrete(limits = as.character(data[1:99,1])) + scale_fill_manual(values=c("lightcoral", "lemonchiffon4", "lightblue3")) ## could modify colors

ggsave("methheatmap.jpg",plot=p)